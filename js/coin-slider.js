...
(function($) {
	...
	$.fn.coinslider= $.fn.CoinSlider = function(options){
		...
		init = function(el){
			...
			// create title bar
			$('#'+el.id).append("<div class='cs-title' id='cs-title-"+el.id+"' style='position: absolute; bottom:39px; left: -252px; z-index: 1000;'></div>");
		...
		$.transition = function(el,direction){
			...
			if(titles[el.id][imagePos[el.id]]){
				$('#cs-title-'+el.id).animate({ 'left' : '-1030px' }, params[el.id].titleSpeed, function(){$(this).html(titles[el.id][imagePos[el.id]]).animate({ 'left' : '-252px' }, params[el.id].titleSpeed, 'easeOutBack')});			
			} else {
	...
	$.fn.coinslider.defaults = {	
		...
		height: 430, // height of slider panel
